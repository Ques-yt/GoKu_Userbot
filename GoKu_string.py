...
def main():
    print("T E A M    G O K U B O T   ! !")
    print("Hello!! Welcome to GoKuBot Session Generator\n")
...
def generate_pyro_session():
    ...
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
    with PClient(name="helluser", api_id=APP_ID, api_hash=API_HASH, in_memory=True) as gokubot:
        print("\nYour HellBot Session Is sent in your Telegram Saved Messages.")
        gokubot.send_message(
            "me",
            f"#GOKUBOT #PYROGRAM\n\n`{gokubot.export_session_string()}`",
        )


def generate_telethon_session():
    print("\nTelethon Session For GoKuBot!")
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
    with TelegramClient(StringSession(), APP_ID, API_HASH) as gokubot:
        print("\nYour GoKu Session Is sent in your Telegram Saved Messages.")
        gokubot.send_message(
            "me",
            f"#GOKUBOT #TELETHON \n\n`{gokubot.session.save()}`",
        )


def generate_insta_session():
    print("Instagram Session For gokuBot!")
    cl = IClient()
...
def hellbot_session(session):
    ...
    error_msg = "Error in generating session! Report it in GoKu Chats"
